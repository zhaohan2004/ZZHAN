...
type Article struct {
	...
	CoverImage   string         `gorm:"type:varchar(255);comment:封面图 URL（空则前端渐变生成）" json:"cover_image"`
	CategoryID   uint64         `gorm:"type:bigint unsigned;not null;index;comment:所属分类ID" json:"category_id"`
	AuthorID     uint64         `gorm:"type:bigint unsigned;not null;index;comment:作者 = 后台管理员ID" json:"author_id"`
	...
	CommentCount uint           `gorm:"type:int unsigned;not null;default:0;comment:评论数(冗余计数)" json:"comment_count"`
